#!/usr/bin/env python3
"""
Reorganize database.py to fix function definition ordering and seed_database closure.
"""

with open("database.py", "r", encoding="utf-8") as f:
    code = f.read()

# Let's clean database.py
split_marker = "# ==================== TOHUMLAMA (SEED DATA) ===================="
if split_marker in code:
    before_seed = code.split(split_marker)[0]
else:
    before_seed = code.split("def seed_database():")[0]

# Now let's append all project & tag & checklist functions cleanly, then seed_database
clean_tail = """
...
new_code = before_seed.strip() + "\n\n" + clean_tail.strip() + "\n"

with open("database.py", "w", encoding="utf-8") as f:
    f.write(new_code)

print("✓ database.py reorganized and syntax cleaned!")
